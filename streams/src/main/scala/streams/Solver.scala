...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val expl = explored + initial.head._1
      val nOnly = newNeighborsOnly(neighborsWithHistory(initial.head._1,initial.head._2),expl)

      initial.head #:: from(initial.tail++nOnly,expl)
    }
  }
